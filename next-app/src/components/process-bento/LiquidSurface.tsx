...
export interface LiquidSurfaceProps {
  ...
  heading?: string;
  showCursor?: boolean;
  /** When true, the WebGL render loop stops (off-screen, hidden tab, etc.). */
  paused?: boolean;
  /** Element that receives pointer events (e.g. the bento tile). Falls back to canvas parent. */
...
class TouchTexture {
  ...
  constructor(touchStrength = 1) {
    this.touchStrength = touchStrength;
    this.size = 64;
    this.width = this.height = this.size;
    this.maxAge = 64;
    this.radius = 0.25 * this.size * touchStrength;
  ...
  addTouch(point: { x: number; y: number }) {
    ...
    if (last) {
      ...
      vx = dx / d;
      vy = dy / d;
      force = Math.min(dd * 20000 * this.touchStrength, 2.0 * this.touchStrength);
    }
...
class WebGLApp {
  ...
  setColorScheme(scheme: number) {
    ...
    if (scheme === 1 || scheme === 6 || scheme === 7 || scheme === 8 || scheme === 5) {
      const bgHex = this.props.darkNavyColor
        ? parseInt(this.props.darkNavyColor.replace('#', ''), 16)
        : 0x07111f;
      this.scene.background = new THREE.Color(bgHex);
      const darkRgb = hexToRgb(this.props.darkNavyColor || '#07111f');
  ...
  dispose() {
    ...
    const material = this.gradientBackground.mesh?.material;
...
export default function LiquidSurface({
  ...
  const interactionRootRef = useRef(interactionRoot);
  ...
  useEffect(() => {
    ...
    const app = new WebGLApp(canvasContainer, {
      speed,
      intensity,
      grainIntensity,
      zoom,
    ...
    const onResize = () => {
      if (canvasContainer) {
        app.onResize(canvasContainer.clientWidth, canvasContainer.clientHeight);
      }
    };
    ...
    const interactionEl =
      interactionRootRef.current ?? canvasContainer.parentElement ?? canvasContainer;
    ...
    const timeoutId = window.setTimeout(onResize, 50);
  ...
  useEffect(() => {
    ...
    if (scheme !== undefined) app.setColorScheme(scheme);
    if (colors) app.updateColors(colors);
  ...
  const handleMouseEnter = () => {
    if (showCursor) {
      gsap.to(cursorRef.current, { width: 50, height: 50, borderWidth: 3, duration: 0.2 });
    }
  };
  const handleMouseLeave = () => {
    if (showCursor) {
      gsap.to(cursorRef.current, { width: 40, height: 40, borderWidth: 2, duration: 0.2 });
    }
  };
  ...
  return (
    <div
      ...
      onMouseEnter={showCursor ? handleMouseEnter : undefined}
      onMouseLeave={showCursor ? handleMouseLeave : undefined}
    >
      {showCursor ? (
        <div
          ref={cursorRef}
          className="pointer-events-none fixed top-0 left-0 z-[1000] flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 border-white mix-blend-difference"
        >
      ...
      {heading ? (
        <div className="pointer-events-none absolute inset-0 z-10 flex h-full w-full items-center justify-center text-center text-white">
          <h1
