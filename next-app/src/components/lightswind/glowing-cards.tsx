...
export interface GlowingCardProps {
  ...
  hoverEffect?: boolean;
  /**
   * `panel` — default padded card chrome (Lightswind-style).
   * `plain` — no background/border/padding; fills parent so outer shell (e.g. `.stack-card`) keeps layout.
   */
  surface?: "panel" | "plain";
}
  ...
  return (
    <div
      className={cn(
        ...
        isPlain
          ? cn(
              "flex min-h-0 min-w-0 flex-1 flex-col rounded-[inherit]",
              "border-0 bg-transparent p-0 shadow-none",
              hoverEffect &&
                "transition-[box-shadow,border-color] duration-300 ease-out hover:z-[1] hover:shadow-[0_0_0_1px_var(--glow-color),0_0_12px_-2px_var(--glow-color)]",
            )
          : cn(
              "flex min-h-0 min-w-[14rem] flex-1 rounded-2xl border bg-background p-6",
              "transition-[box-shadow,border-color,background-color] duration-300 ease-out",
              hoverEffect &&
                "hover:z-[1] hover:shadow-[0_0_0_1px_var(--glow-color),0_0_14px_-4px_var(--glow-color)]",
            ),
        className,
      )}
      style={
  ...
  return (
    ...
    >
      <div
        ...
        data-lw-hover={enableHover ? "true" : "false"}
